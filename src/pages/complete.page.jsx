...
function CompletePage() {
  ...
  if (isError) {
    return (
      <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md text-center">
        ...
        <p className="mb-4">We couldn't process your payment information. Please try again or contact support.</p>
        <Button asChild className="mt-6">
          <Link to={`/booking/payment?bookingId=${data?.bookingId || ""}`}>Return to Payment Page</Link>
        </Button>
      </div>
    );
  }
  ...
  if (data?.status === "complete") {
    ...
    return (
      <section id="success" className="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md my-8">
        ...
        <div className="mt-6 border rounded-lg overflow-hidden">
          <div className="relative h-48">{data.hotel.image && <img src={data.hotel.image} alt={data.hotel.name} className="w-full h-full object-cover" />}</div>
          <div className="p-4 bg-gray-50">
            ...
            {data.hotel.rating && (
              <div className="flex items-center mb-2">
                ...
                {data.hotel.reviews && <span className="text-gray-500 text-sm ml-1">({data.hotel.reviews} reviews)</span>}
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-gray-50 p-4 rounded-md">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium capitalize">{data.booking.paymentMethod ? data.booking.paymentMethod.replace("_", " ").toLowerCase() : "card"}</p>
              </div>
              <div>
                <p className="text-gray-600 text-sm">Payment Status</p>
                <p className="font-medium text-green-600">{data.booking.paymentStatus}</p>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md text-center">
      ...
      <p className="mb-4">We couldn't determine the status of your payment. If you completed a booking, please check your email for confirmation.</p>
      <Button asChild className="mt-6">
        <Link to="/">Return to Home</Link>
      </Button>
    </div>
