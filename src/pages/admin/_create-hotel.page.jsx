...
function CreateHotelPage() {
  ...
  return (
    <main className="px-4 py-8">
      ...
      <form onSubmit={handleCreateHotel} className="mt-4 w-1/4 flex flex-col gap-4">
        <div className="flex flex-col gap-2">
          <Label htmlFor="name">Name</Label>
          <Input id="name" value={formData.name.value} onChange={handleValueChange} name="name" className="block" />
          <span className="text-red-500">{formData.name.error}</span>
        </div>
        <div className="flex flex-col gap-2">
          <Label htmlFor="description">Description</Label>
          <Input id="description" value={formData.description.value} onChange={handleValueChange} name="description" className="block" />
          <span className="text-red-500">{formData.description.error}</span>
        </div>
        <div className="flex flex-col gap-2">
          <Label htmlFor="image">Image</Label>
          <Input id="image" value={formData.image.value} onChange={handleValueChange} name="image" className="block" />
          <span className="text-red-500">{formData.image.error}</span>
        </div>
        <div className="flex flex-col gap-2">
          <Label htmlFor="location">Location</Label>
          <Input id="location" value={formData.location.value} onChange={handleValueChange} name="location" className="block" />
          <span className="text-red-500">{formData.location.error}</span>
        </div>
        <div className="flex flex-col gap-2">
          <Label htmlFor="price">Price</Label>
          <Input id="price" type="number" value={formData.price.value} onChange={handleValueChange} name="price" className="block" />
